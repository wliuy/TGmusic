...
const COMMIT_MSG = 'feat: Sarah MUSIC 9.0.4 (D1 无损重构，修复歌手名显示异常)';
const files = {};
...
files['sw.js'] = `const CACHE_NAME = 'sarah-music-v904';
...
<body>
    ...
    <div class="desktop-container" id="main-ui">
        <header class="header-stack">
            ...
            <p class="brand-sub">Premium Music Hub | v9.0.4</p>
        ...
        <div id="admin-box">
            <div class="admin-header">
                <div class="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span class="text-[10px] font-black text-white/40 bg-white/5 px-2 py-0.5 rounded tracking-wider">v9.0.4</span>
                </div>
    ...
    <script>
        ...
        function showSarahDialog(title, msg, isInput, def, cb) { 
            ...
            document.getElementById('dialog-confirm').onclick = () => { ov.classList.remove('active'); cb(isInput ? inp.value : true); }; ov.classList.add('active');
        }
        ...
        function openPlaylistSelector(fid) {
            document.getElementById('playlist-selector-list').innerHTML = libState.playlists.map(pl => {
                ...
                return \`<div onclick="addToPlaylist('\${pl.id}', '\${fid}')" class="p-4 bg-white/5 rounded-2xl flex justify-between items-center cursor-pointer hover:bg-white/10 \text{ex?'opacity-30':''}"><span>\${pl.name}</span>\${ex?'<span class="text-[10px]">已在</span>':'+'}</div>\`;
            }).join('');
...
try {
    ...
    try {
        ...
        console.log('\n✅ Sarah MUSIC 9.0.4 构建成功。封面已改为由 Telegram 托管并以 URL 形式存储，彻底根治 SQLITE_TOOBIG。');
