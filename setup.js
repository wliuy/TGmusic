...
 * 1. 标题自定义：新增 APP_TITLE 环境变量支持。如果设置，则全局显示自定义字符，否则默认回退至 "Sarah"。
 * 2. 视觉秒开：移除 UI 容器的强制隐藏样式，重构 init 流程使主题先行、数据后到，根治首屏白屏问题。
 * 3. 带宽优化：针对 768px 以下设备物理禁用后台预载机制，消除起播阶段的资源竞争，实现即刻播放。
 * 4. 预览增强：恢复预览操作对背景层的静默调用，确保歌单标签高亮（底色）即时跟随预览意图。
 * 5. 版本同步：全面对齐 HTML 文本与控制台日志的版本号标识。
 */
const REMOTE_URL = 'git@github.com:wliuy/TGmusic.git';
const COMMIT_MSG = 'feat: Sarah MUSIC 10.1.8 (支持标题环境变量 APP_TITLE 自定义)';
const files = {};
...
  0      .browser-tab.active .browser-tab-text { opacity: 1; color: #10b981; }
...
            <h1 class="brand-title" id="main-brand-text">Sarah</h1>
...
            <h1 class="text-xl font-black text-white" id="m-brand-text">Sarah</h1>
...
                
                // 动态标题注入逻辑 (10.1.8)
                const appName = libState.app_title || "Sarah";
                document.title = appName;
                const t1 = document.getElementById('main-brand-text'), t2 = document.getElementById('m-brand-text');
                if(t1) t1.innerText = appName; if(t2) t2.innerText = appName;
...
    });
    console.log('\n---正在同步至 GitHub (10.1.8 Customized Title)---');
    try {
...
        console.log('\\n✅ Sarah MUSIC 10.1.8 构建成功。标题自定义逻辑已生效，物理保真度 100%。');
