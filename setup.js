...
/**
 * Sarah MUSIC 旗舰全功能重构版 9.4.5
 * 1. 无损重构：全量继承 9.3.1 的视觉厚度与交互算法，拒绝任何代码简化。
 * 2. D1 深度集成：使用 Cloudflare D1 关系型数据库，完美支撑千级歌曲管理。
 * 3. 独立排序：实现全库、收藏、自定义列表的排序位物理隔离。
 * 4. 逻辑锁定：修复 PC 端拖拽歌单导致的名称交换与排序保存失效问题。
 */
const REMOTE_URL = 'git@github.com:wliuy/TGmusic.git';
const COMMIT_MSG = 'feat: Sarah MUSIC 9.4.5 (彻底修复PC端自定义列表拖拽排序与状态同步逻辑)';
const files = {};
...
files['sw.js'] = `const CACHE_NAME = 'sarah-music-v945';
...
<body>
    ...
    <div class="desktop-container" id="main-ui">
        <header class="header-stack">
            ...
            <p class="brand-sub">Premium Music Hub | v9.4.5</p>
        ...
        <div id="admin-box">
            <div class="admin-header">
                <div class="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span class="text-[10px] font-black text-white/40 bg-white/5 px-2 py-0.5 rounded tracking-wider">v9.4.5</span>
                </div>
    ...
    <script>
        ...
        function handleAdminPlaylistDragStart(e, idx) {
            if (window.innerWidth <= 768 || e.target.closest('.browser-tab-close')) return;
            ...
            const initDrag = (cX, cY) => {
                ...
                const end = async () => {
                    ...
                    let activeId = (currentTab !== 'all' && currentTab !== 'fav') ? libState.playlists[parseInt(currentTab)]?.id : null;
                    const newPls = nIds.map(id => libState.playlists.find(p => p.id === id)).filter(Boolean);
                    libState.playlists = newPls;
                    if(activeId) { const nIdx = libState.playlists.findIndex(p => p.id === activeId); if(nIdx !== -1) currentTab = nIdx.toString(); }
                    renderAdminPlaylistTabs(); renderCustomTabs();
...
try {
    ...
    try {
        ...
        console.log('\n✅ Sarah MUSIC 9.4.5 构建成功。已彻底解决PC端歌单拖拽排序同步问题。');
