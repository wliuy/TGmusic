...
const COMMIT_MSG = 'feat: Sarah MUSIC 9.0.9 (D1 无损重构，修复 Node 构建转义报错)';
const files = {};
...
files['functions/api/songs.js'] = `export async function onRequest(context) {
  ...
  try {
    ...
    const mappings = await env.DB.prepare("SELECT * FROM playlist_mapping ORDER BY sort_order ASC").all();
...
files['sw.js'] = `const CACHE_NAME = 'sarah-music-v909';
...
<body>
    ...
    <div id="playlist-selector-modal" class="fixed inset-0 bg-black/60 backdrop-blur-xl z-[3000] hidden items-center justify-center" onclick="closePlaylistSelector()">
        <div class="playlist-select-box w-[300px] bg-[#1e293b] border border-white/10 rounded-3xl p-6 shadow-2xl" onclick="event.stopPropagation()">
    ...
    <div class="desktop-container" id="main-ui">
        <header class="header-stack">
            ...
            <p class="brand-sub">Premium Music Hub | v9.0.9</p>
    ...
    <div id="m-player" class="mobile-player-container">
        <header class="m-header">
            ...
            <div onclick="toggleAdmin(true)" class="btn-round !bg-transparent"><svg class="w-7 h-7" fill="none" stroke="white" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg></div>
        </header>
        <main class="m-main">
        ...
        <div id="admin-box">
            <div class="admin-header">
                <div class="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span class="text-[10px] font-black text-white/40 bg-white/5 px-2 py-0.5 rounded tracking-wider">v9.0.9</span>
                </div>
    ...
    <script>
        ...
        function updateBackground(isForceRandom = false) { 
            ...
            document.querySelectorAll('.btn-round').forEach(el => {
                if(!el.classList.contains('!bg-white/10') && !el.parentElement.classList.contains('m-header')) { el.style.background = isMob ? '#ffffff' : theme.accent; el.style.color = isMob ? '#4d7c5f' : 'white'; }
        ...
        function handleAdminDragStart(e, idx, isTouch) {
            ...
            const initDrag = (cX, cY) => {
                ...
                const end = async () => {
                    ...
                    const pid = currentTab === 'all' ? 'all' : (currentTab === 'fav' ? 'fav' : libState.playlists[parseInt(currentTab)].id);
                    await dbOp('update_order', { playlist_id: pid, ids: nIds }); init();
        ...
        async function updateSongInfo(fid, field, val) { const s = db.find(x => x.file_id === fid); if(s) { s[field] = val; await dbOp('update_song', s); init(); } }
        
        function deleteSong(fid) { 
            showSarahDialog("删除确认", "确定将此旋律从全库中永久抹去吗？", false, null, async (y) => { if(y) { await dbOp('delete_song', { file_id: fid }); init(); } }); 
        }
        ...
        function openPlaylistSelector(fid) {
            document.getElementById('playlist-selector-list').innerHTML = libState.playlists.map(pl => {
                ...
                return \`<div onclick="addToPlaylist('\${pl.id}', '\${fid}')" class="p-4 bg-white/5 rounded-2xl flex justify-between items-center cursor-pointer hover:bg-white/10 \${ex?'opacity-30':''}"><span>\${pl.name}</span>\${ex?'<span class="text-[10px]">已在</span>':'+'}</div>\`;
            }).join('');
        ...
        async function addToPlaylist(pid, fid) { await dbOp('add_to_playlist', { playlist_id: pid, file_id: fid }); closePlaylistSelector(); showMsg("✨ 已分发"); init(); }

        function addPlaylistPrompt() { showSarahDialog("新歌单", "名称：", true, "", async (n) => { if(n) { await dbOp('add_playlist', { name: n }); init(); } }); }
        ...
        function deletePlaylist(idx) { showSarahDialog("删除", "确定删除此列表吗？", false, null, async (y) => { if(y) { await dbOp('delete_playlist', { id: libState.playlists[idx].id }); init(); } }); }
        ...
        async function handleUp() {
            ...
            const btn = document.getElementById('auto-sync-trigger'); btn.disabled = true;
...
try {
    ...
    try {
        ...
        console.log('\n✅ Sarah MUSIC 9.0.9 构建成功。已修复 Node 构建时的 SyntaxError 转义问题。');
