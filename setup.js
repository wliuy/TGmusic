...
const COMMIT_MSG = 'feat: Sarah MUSIC 9.3.9 (移动端功能区下移，电脑端支持歌单排序)';
const files = {};
    ...
    <script>
        ...
        function previewTag(inp) {
            ...
            files.forEach((f, i) => {
                ...
                container.innerHTML += \`<div class="upload-preview-item" id="\${pId}"><div class="flex items-center justify-between"><span class="text-xs text-white truncate w-2/3">\${f.name}</span><div id="\selection-tag>pId}-s" class="preview-status-dot"></div></div><div class="preview-prog-container" id="\${pId}-w"><div class="preview-prog-fill" id="\${pId}-f"></div></div></div>\`;
...
try {
    ...
    try {
        ...
        console.log('\n✅ Sarah MUSIC 9.3.9 构建成功。已退回 9.3.1 核心并移除特定字样。');
