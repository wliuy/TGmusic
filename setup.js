...
files['functions/api/manage.js'] = `export async function onRequest(context) {
  ...
  try {
    ...
    if (action === 'get_lrc') {
      ...
      try { await env.DB.batch(statements); } catch(e) { console.error("Persistence failed: " + e.message); }
...
files['manifest.json'] = `{
  ...
  "icons": [
    { "src": "https://tc.ayang.cc.cd/file/logo/tgmusic.png", "sizes": "144x144", "type": "image/png", "purpose": "any maskable" },
    { "src": "https://tc.ayang.cc.cd/file/logo/tgmusic.png", "sizes": "152x152", "type": "image/png", "purpose": "any maskable" },
    { "src": "https://tc.ayang.cc.cd/file/logo/tgmusic.png", "sizes": "192x192", "type": "image/png", "purpose": "any maskable" },
    { "src": "https://tc.ayang.cc.cd/file/logo/tgmusic.png", "sizes": "512x512", "type": "image/png", "purpose": "any maskable" }
  ]
}`;
...
<head>
    ...
    <link rel="apple-touch-icon" sizes="192x192" href="https://tc.ayang.cc.cd/file/logo/tgmusic.png">
    <title>Sarah</title>
    <link rel="icon" type="image/png" sizes="192x192" href="https://tc.ayang.cc.cd/file/logo/tgmusic.png">
    ...
    <style>
        ...
        .content-layout { flex: 1 1 0%; min-height: 0; display: grid; grid-template-columns: minmax(0, 0.65fr) minmax(0, 1fr) minmax(0, 1fr); grid-template-rows: minmax(0, 1fr); gap: 24px; overflow: hidden; margin: 10px 0; max-height: 55%; }
        ...
        .song-actions { position: absolute; right: 12px; display: flex; gap: 8px; opacity: 0; transition: 0.2s; pointer-events: auto; }
        .song-item:hover .song-actions { opacity: 1; }
        .action-btn { width: 28px; height: 28px; display: grid; place-items: center; border-radius: 8px; background: rgba(255,255,255,0.15); color: inherit; transition: 0.2s; }
        .action-btn:hover { background: var(--dynamic-accent); color: white !important; transform: scale(1.1); }
        .action-btn.is-fav { color: #ef4444 !important; }
        .action-btn.is-fav:hover { background: rgba(239, 68, 68, 0.15); color: #ef4444 !important; }
        ...
        .dot { 
            ...
            border-radius: 50%; 
            cursor: grab; z-index: 100 !important; 
            transition: transform 0.2s;
        }
        .dot:hover { transform: translateY(-50%) scale(1.15); }
        ...
        .m-pl-card { 
            flex-shrink: 0; height: 34px; min-width: 65px; padding: 0 16px; 
            border-radius: 10px 10px 0 0; background: rgba(255, 255, 255, 0.06); 
            border: 1px solid rgba(255, 255, 255, 0.1); border-bottom: none; 
            display: flex; align-items: center; justify-content: center; 
            margin-right: 3px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        ...
        .m-list-search-box { width: 100%; height: 46px; background: rgba(255, 255, 255, 0.08); border: 1.5px solid rgba(255, 255, 255, 0.15); border-radius: 16px; padding: 0 45px 0 20px; color: white; font-size: 14px; font-weight: 700; outline: none; }
        ...
        .admin-btn-icon { width: 38px; height: 38px; display: grid; place-items: center; background: rgba(255, 255, 255, 0.1); border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.15); transition: 0.25s; cursor: pointer; color: white; backdrop-filter: brightness(1.1); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1); padding: 0 !important; }
        .admin-btn-icon:hover { background: rgba(255, 255, 255, 0.2); border-color: rgba(255, 255, 255, 0.3); transform: scale(1.05); }
        .admin-btn-icon:active { transform: scale(0.92); }
        ...
        /* 认证层样式 */
        #auth-gate { position: fixed; inset: 0; z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 24px; transition: all 1.2s cubic-bezier(0.4, 0, 0.2, 1); }
        .auth-card {
            width: 100%; max-width: 380px; border-radius: 2rem; padding: 4.5rem 3rem 4rem;
            text-align: center; display: flex; flex-direction: column; align-items: center;
            background: rgba(255, 255, 255, 0.12); backdrop-filter: blur(40px); -webkit-backdrop-filter: blur(40px);
            border: 1px solid rgba(255, 255, 255, 0.2);
            box-shadow: 0 50px 100px rgba(0, 0, 0, 0.12), inset 0 1px 0 rgba(255,255,255,0.15);
            transition: all 1.2s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .auth-title {
            font-size: 1.5rem; font-weight: 900; margin-bottom: 0.6rem; letter-spacing: 0.1em; text-transform: uppercase;
        }
        .auth-subtitle {
            font-size: 0.8rem; font-weight: 500; margin-bottom: 3rem; letter-spacing: 0.06em; opacity: 0.6;
        }
        .auth-input {
            width: 100%; border-radius: 1rem; padding: 1.15rem 1.5rem; font-size: 1.05rem; font-weight: 600;
            outline: none; transition: all 0.3s ease; text-align: center; letter-spacing: 0.2em;
            border: 1.5px solid rgba(255, 255, 255, 0.15);
        }
        .auth-input:focus { border-color: rgba(255, 255, 255, 0.4); transform: scale(1.02); }
        .auth-btn {
            width: 100%; padding: 1.15rem; border-radius: 1rem; font-weight: 800; font-size: 0.85rem;
            letter-spacing: 4px; transition: all 0.3s ease; border: none; cursor: pointer;
            text-transform: uppercase; margin-top: 1.5rem;
        }
        .auth-btn:active { transform: scale(0.96); }
        /* 桌面端主题渐变背景 */
        @media (min-width: 769px) {
            #auth-gate { background: var(--auth-bg, linear-gradient(135deg, #f0fdf4, #064e3b)); }
            .auth-title { color: var(--auth-text, #1e293b); }
            .auth-subtitle { color: var(--auth-text, #1e293b); }
            .auth-input { background: rgba(255, 255, 255, 0.25); color: var(--auth-text, #1e293b); }
            .auth-input::placeholder { color: var(--auth-text-muted, rgba(30,41,59,0.35)); }
            .auth-btn { background: var(--auth-accent, #10b981); color: #fff; box-shadow: 0 12px 30px var(--auth-accent-shadow, rgba(16,185,129,0.3)); }
        }
        /* 手机端主题色背景 */
        @media (max-width: 768px) {
            #auth-gate { background: var(--auth-deep, #064e3b); backdrop-filter: blur(80px); -webkit-backdrop-filter: blur(80px); }
            .auth-title { color: #fff; }
            .auth-subtitle { color: rgba(255,255,255,0.5); }
            .auth-input { background: rgba(255, 255, 255, 0.08); color: #fff; }
            .auth-input::placeholder { color: rgba(255,255,255,0.25); }
            .auth-btn { background: #fff; color: var(--auth-deep, #064e3b); box-shadow: 0 12px 30px rgba(0,0,0,0.15); }
        }

        /* 窗口自适应：中等宽度桌面仅收紧间距与封面，保持三栏布局，歌词始终可见 */
        @media (max-width: 1150px) {
            .desktop-container { padding: 16px 22px; gap: 12px; }
            .content-layout { gap: 16px; }
            .cover-container { width: min(10rem, 26vh); height: min(10rem, 26vh); }
...
<body>
    ...
    <div id="sarah-dialog" class="sarah-dialog-overlay fixed inset-0 bg-black/60 backdrop-blur-2xl z-[3000] hidden items-center justify-center" onclick="closeSarahDialog()">
        <div class="sarah-dialog-box w-[320px] bg-[#1e293b]/95 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl scale-95 opacity-0 transition-all duration-300 pointer-events-none" onclick="event.stopPropagation()">
            <h4 id="dialog-title" class="text-white font-black text-xl mb-2 text-center tracking-tight">提示</h4>
            <p id="dialog-msg" class="text-white/60 text-xs mb-8 text-center leading-relaxed px-4"></p>
            <div id="dialog-input-wrap" class="hidden mb-8">
                <input id="dialog-input" class="w-full bg-white/5 p-4 rounded-2xl text-white text-sm outline-none border border-white/10 focus:border-[#4d7c5f]/50 focus:bg-white/10 transition-all shadow-inner" placeholder="请输入...">
            </div>
            <div class="flex gap-3">
                <button id="dialog-cancel" onclick="closeSarahDialog()" class="flex-1 py-4 bg-white/5 text-white/50 hover:text-white hover:bg-white/10 rounded-2xl text-[11px] font-black transition-all active:scale-95">取消</button>
                <button id="dialog-confirm" class="flex-1 py-4 bg-[#4d7c5f] hover:bg-[#5a8f6e] text-white rounded-2xl text-[11px] font-black shadow-lg transition-all active:scale-95">确认</button>
            </div>
        </div>
    </div>
    ...
    <div class="desktop-container" id="main-ui">
        ...
        <main class="content-layout">
            ...
            <section class="panel-box flex flex-col overflow-hidden">
                ...
                <div id="list-view" class="flex-1 overflow-y-auto px-1 pb-3 custom-scroll" onscroll="handleScroll(this)"></div>
            </section>
        ...
        <footer class="footer-bar">
            ...
            <div class="scrubber-area">
                ...
                <div id="pc-scrubber" class="rail" onmousedown="handleMouseSeekStart(event)">
                    <div class="buffer-fill" id="prog-buffer"></div>
            ...
            <div class="volume-control">
                ...
                <div id="pc-vol-rail" class="volume-rail" onmousedown="handleMouseVolStart(event)"><div class="fill" id="vol-bar" style="width: 70%"><div class="dot !w-3 !h-3 !-right-1.5" id="vol-dot" style="box-shadow:none !important"></div></div></div>
            </div>
        </footer>
    </div>
        ...
        <footer class="m-controls-capsule">
            <div class="m-controls">
                ...
                <div class="m-btn-row">
                    ...
                    <div onclick="toggleMobileDrawer(true)" class="btn-round !bg-white !text-[#4d7c5f]"><svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="4.2"><path d="M4 6h16M4 12h16M4 18h16"></path></svg></div>
                </div>
            </div>
        </footer>
    </div>
    ...
    <div id="m-drawer" class="m-drawer">
        ...
        <div id="m-list-view" class="flex-1 overflow-y-auto custom-scroll text-white px-4 pb-10" onscroll="handleScroll(this)"></div>
    </div>
        ...
        <div id="admin-box">
            <div class="admin-header">
                ...
                <div id="admin-header-center">
                    ...
                    <div id="upload-area" class="hidden"><div class="admin-console-box flex items-center gap-4"><input type="file" id="f-in" multiple onchange="previewTag(this)" style="display:none"><label for="f-in" class="upload-card"><div class="upload-hint"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1M12 4v12M16 8l-4-4-4 4"/></svg><span id="file-count-tip">点击/拖拽同步</span></div></label><button onclick="switchAdminList('logs')" class="bg-white/15 text-white px-5 py-2.5 rounded-xl font-black text-[11px] hover:bg-white/20 transition-all">上传日志</button></div></div>
                </div>
                <div class="admin-action-bar">
                    <button onclick="toggleSleepArea()" class="admin-btn-icon" title="定时暂停"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>
                    <button onclick="toggleUploadArea()" class="admin-btn-icon" title="同步音乐"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1M12 4v12M16 8l-4-4-4 4"/></svg></button>
                    <button onclick="toggleAdmin(false)" class="admin-btn-icon !bg-white/20" title="关闭设置"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>
                </div>
            </div>
            <div class="admin-content custom-scroll">
                <div id="upload-preview-list" class="space-y-2 mb-1"></div>
                <div class="flex items-center mb-1"><div id="admin-playlist-tabs" class="admin-tabs-nav flex-1"></div></div>
    ...
    <script>
        ...
        const modes = ['list', 'single', 'random'], DEFAULT_LOGO = 'https://tc.ayang.cc.cd/file/logo/tgmusic.png';
        const solaraTheme = [
        ...
        function applyAuthTheme() {
            ...
            gate.style.setProperty('--auth-text-muted', 'rgba(30,41,59,0.35)');
          ...
          try {
            ...
            if(ap) {
              ...
              if (curFid) {
                  ...
                  if (newIndex !== -1) { ap.list.index = newIndex; }
              }
            }
        ...
        function setupPlayer() {
            ...
            ap.audio.addEventListener('progress', () => {
                ...
                if (audio.buffered.length > 0) {
                    ...
                    if (duration > 0) {
                        const p = (bufferedEnd / duration) * 100 + "%";
                        ['prog-buffer', 'm-prog-buffer'].forEach(id => {
            ...
            ap.on('play', () => { 
                ...
                if (cur) {
                    ...
                    if (fid && fid !== globalPlayingId) {
                        globalPlayingId = fid;
                        refreshUIMetaByAudio(cur);
                    }
            ...
            ap.on('listswitch', (data) => { 
                ...
                const audio = ap.list.audios[targetIdx];
                if (audio) {
                    const fid = new URLSearchParams(audio.url.split('?')[1]).get('file_id');
                    if (fid && fid !== globalPlayingId) {
        ...
        function updateMediaSession() { 
            ...
            navigator.mediaSession.playbackState = ap.paused ? 'paused' : 'playing';
            ...
            navigator.mediaSession.setActionHandler('seekto', (details) => { if (details.seekTime) ap.seek(details.seekTime); });
        }
        ...
        function updateBackground(isForceRandom = false) { 
            ...
            listTabs.forEach(el => {
                ...
                const isActive = (tabId === 'tab-play' && currentTab === 'all') || (tabId === 'tab-fav' && currentTab === 'fav') || (tabId === 'tab-pl-' + currentTab) || (el.dataset.id === currentTab);
                ...
                else { if(isActive) { el.style.cssText = "background:rgba(255, 255, 255, 0.18) !important; color:white !important; font-weight:900;"; } else { el.style.cssText = ""; } }
            });
        ...
        async function refreshUIMetaAt(idx) {
            const song = db[idx]; if (!song) return;
            globalPlayingId = song.file_id;
        ...
        function updateHighlights(cid, shouldScroll = false) {
            ...
            document.querySelectorAll('.song-item').forEach(el => {
                const isActive = el.dataset.id === cid;
                el.classList.toggle('active', isActive);
                if (isActive && shouldScroll) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            });
        }
        ...
        async function toggleLikeById(fid) {
            ...
            document.querySelectorAll(\`.song-item[data-id="\${fid}"]\`).forEach(el => {
                ...
                if(btn) { 
                    btn.classList.toggle('is-fav', isAdding); 
                    btn.querySelector('svg').setAttribute('fill', isAdding ? 'currentColor' : 'none'); 
                }
            });
            updateHighlights(fid, false);
        ...
        async function deleteSongById(fid) {
            ...
            showSarahDialog("删除确认", \`确定将此旋律从\${pid === 'all' ? '全库永久抹去' : '当前歌单移除'}吗？\`, false, null, async (y) => { 
                if(y) { 
                    await dbOp('delete_song', { file_id: fid, playlist_id: pid }); 
                    await silentRefresh();
                    renderAllLists();
                } 
            }); 
        ...
        function showSarahDialog(title, msg, isInput, def, cb) { 
            ...
            if(isInput) { 
                ...
                inp.value = def || ""; 
                inp.placeholder = msg.replace('：', '') || "请输入...";
            ...
            document.getElementById('dialog-confirm').onclick = () => { 
                ...
                cb(isInput ? inp.value : true); 
            }; 
            ov.style.display = 'flex';
            ov.classList.remove('hidden'); 
            setTimeout(() => { ov.classList.add('active'); box.style.transform = 'scale(1)'; box.style.opacity = '1'; box.style.pointerEvents = 'auto'; }, 10);
        }
        function closeSarahDialog() { 
            const ov = document.getElementById('sarah-dialog'); 
            const box = ov.querySelector('.sarah-dialog-box');
            ov.classList.remove('active'); 
            box.style.transform = 'scale(0.95)';
        ...
        function renderAdminPlaylistTabs() {
            ...
            libState.playlists.forEach((pl, i) => { 
                const hasLeft = i > 0; const hasRight = i < libState.playlists.length - 1;
                const arrowHtml = isPC ? \`<div class="browser-tab-arrow">\${hasLeft ? '<span onclick="event.stopPropagation(); movePlaylist(' + i + ', -1)">←</span>' : ''}\${hasRight ? '<span onclick="event.stopPropagation(); movePlaylist(' + i + ', 1)">→</span>' : ''}</div>\` : '';
                html += \`<div class="browser-tab \${currentAdminTab === pl.id ? 'active' : ''}" data-plid="\${pl.id}" onclick="switchAdminList('\${pl.id}')" onmousedown="\${isPC ? 'handleAdminPlaylistDragStart(event, '+i+')' : ''}" ondblclick="renamePlaylistPrompt('\${i}')">\${arrowHtml}<span class="browser-tab-text">\${pl.name} <i class="opacity-40 text-[9px] font-black italic">\${pl.ids.length}</i></span><div class="browser-tab-close" onclick="event.stopPropagation(); deletePlaylist('\${i}')"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M6 18L18 6M6 6l12 12"></path></svg></div></div>\`; 
            });
        ...
        async function renderUploadLogs() {
            ...
            container.innerHTML = res.logs.map(log => {
                const dateStr = new Date(log.timestamp + " UTC").toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false });
        ...
        function handleAdminDragStart(e, idx, isTouch) {
            ...
            const initDrag = (cX, cY) => {
                ...
                const end = async () => {
                    ...
                    const pid = (currentAdminTab === 'all' || currentAdminTab === 'fav') ? currentAdminTab : currentAdminTab;
        ...
        async function movePlaylist(idx, dir) {
            ...
            const temp = libState.playlists[idx]; libState.playlists[idx] = libState.playlists[nIdx]; libState.playlists[nIdx] = temp;
            renderAdminPlaylistTabs(); renderCustomTabs(); updateBackground(false);
        ...
        function handleAdminPlaylistDragStart(e, idx) {
            ...
            const initDrag = (cX, cY) => {
                ...
                const offsetLeft = cX - rect.left;
                dragPlaceholder = document.createElement('div');
                dragPlaceholder.className = 'browser-tab is-hidden';
                dragPlaceholder.style.width = rect.width + 'px';
                targetEl.parentNode.insertBefore(dragPlaceholder, targetEl);
                ...
                const move = (me) => {
                    const mX = me.clientX; ghost.style.left = (mX - offsetLeft) + 'px';
                    const hov = document.elementFromPoint(mX, cY)?.closest('.browser-tab');
                    if (hov && hov !== targetEl && hov !== dragPlaceholder && hov.dataset.plid) {
                        const r = hov.getBoundingClientRect();
                        if (mX < r.left + r.width / 2) hov.parentNode.insertBefore(dragPlaceholder, hov);
                        else hov.parentNode.insertBefore(dragPlaceholder, hov.nextSibling);
                    }
                };
                const end = async () => {
                    window.removeEventListener('mousemove', move); window.removeEventListener('mouseup', end);
                    if (dragPlaceholder) { dragPlaceholder.parentNode.insertBefore(targetEl, dragPlaceholder); dragPlaceholder.remove(); }
                    if (ghost) ghost.remove(); targetEl.classList.remove('is-hidden');
                    const nIds = Array.from(document.querySelectorAll('.browser-tab[data-plid]')).map(t => t.dataset.plid);
                    const newPls = nIds.map(id => libState.playlists.find(p => p.id === id)).filter(Boolean);
                    libState.playlists = newPls;
                    renderAdminPlaylistTabs(); renderCustomTabs(); updateBackground(false);
                    await dbOp('update_playlist_order', { ids: nIds });
                };
            ...
            const checkInit = (me) => {
                if (Math.abs(me.clientX - startX) > 5 || Math.abs(me.clientY - startY) > 5) {
                    isMoved = true; window.removeEventListener('mousemove', checkInit); initDrag(me.clientX, me.clientY);
                }
            };
        ...
        function toggleEditMode(idx) {
            ...
            if (isEditing) {
                ...
                btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.121 2.121 0 113 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>';
        ...
        async function addToPlaylist(pid, fid) { 
            const pl = libState.playlists.find(p => p.id === pid);
            if(pl && !pl.ids.includes(fid)) {
                pl.ids.push(fid);
                openPlaylistSelector(fid);
            }
        ...
        function previewTag(inp) {
            ...
            files.forEach((f, i) => {
                ...
                jsmediatags.read(f, { onSuccess: (t) => {
                    ...
                    if(i === files.length - 1) setTimeout(() => handleUp(files), 500);
        ...
        async function handleUp(batchFiles) {
            ...
            const targetPid = (currentAdminTab !== 'all' && currentAdminTab !== 'fav' && currentAdminTab !== 'logs') ? currentAdminTab : null;
            ...
            const worker = async () => {
                while(globalUploadQueue.length > 0) {
                    ...
                    if (!item) break;
                    const sDot = document.getElementById(item.pId + "-s"), pFill = document.getElementById(item.pId + "-f");
                    ...
                    await new Promise(r => {
                        xhr.onload = () => { 
                            ...
                            else { if(sDot) sDot.className = "preview-status-dot error"; }
                            r(); 
                        };
...
try {
    ...
    if (!files['wrangler.toml'] && fs.existsSync(wp)) fs.unlinkSync(wp);
